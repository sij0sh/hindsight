...
test('Pi SDK adapter uses isolated in-memory sessions and exactly the five investigation tools',async t=>{
  ...
  const metrics=await runPiCurator(inv,{sdk,modelRuntime:{}});
...
test('adapter rejects a missing submission, unexpected write tool, and exhausted turn budget',async t=>{
  ...
  await assert.rejects(()=>runPiCurator(inv,{sdk:mock.sdk,modelRuntime:{}}),/without submitting/);
  inv=await makeLedger(t);mock=mockSdk(inv,{extraTool:true});await assert.rejects(()=>runPiCurator(inv,{sdk:mock.sdk,modelRuntime:{}}),/Unexpected curator tools/);
  inv=await makeLedger(t);inv.config.maxTurns=1;mock=mockSdk(inv,{turns:2});await assert.rejects(()=>runPiCurator(inv,{sdk:mock.sdk,modelRuntime:{}}),/maxTurns/);assert.equal(mock.observed.aborted,true);
});
...
test('deadline rejects even if a provider does not settle its prompt after abort',async t=>{
  ...
  await assert.rejects(()=>runPiCurator(inv,{sdk,modelRuntime:{}}),/timeoutMs/);assert.equal(observed.aborted,true);assert.equal(observed.disposed,true);
});
